/*! ----------------------------------------------------------------------------
 * @file    mock_seq.h
 * @brief   Shared call-order sequence counter for the dw1000_sleep ztest
 *          suite (UWB-316).
 *
 ...
 * number, so the test assertions can verify call ORDER (e.g. "WAKEUP toggle
 * happens before dwt_initialise()") and not just call presence/count.
 *
 * @attention
